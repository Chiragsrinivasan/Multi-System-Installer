...
def installer(i,s_host,s_port,s_username,s_password):
    ...
    cmd = f'echo {s_password} | sudo -S apt install ./{i} -y'
...
def remove(i,s_host,s_port,s_username,s_password):
    ssh_client = connectionbuild(s_host,s_port,s_username,s_password)
    cmd = f'echo {s_password} | sudo -S apt remove {i} -y'
    stdin, stdout, stderr = ssh_client.exec_command(cmd)
    output = stdout.read().decode("utf-8")
    print(output)
    x = 0
...
def main():
    ...
    os.system('cls')
    print("\t\t\t_____________________MULTI-SYSTEM_INSTALLER______________________")
    conn = dbcon()
    s_host,s_port,s_username,s_password,s_path =  user(conn)
    conn.close()
